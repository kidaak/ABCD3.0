...
public class ListasDeSeleccion implements IContributor {
	...
	public Control createUIControl(final Composite parent) {
		...
		FormDatas.attach(agregar).atTop(0).atLeft(0).atRight(0);
		
				
				
		Label nombreLista = new Label(agregar, SWT.NORMAL);
        nombreLista.setText("Nombre de la Lista:");
		FormDatas.attach(nombreLista).atTopTo(agregar,15).atLeftTo(agregar, 10); 
		
		Text nombreListaText = new Text(agregar, SWT.NORMAL);
		FormDatas.attach(nombreListaText).atTopTo(nombreLista, 10).atLeftTo(agregar, 10).withWidth(150);
		...
		FormDatas.attach(ordenarLista).atTopTo(nombreListaText,30).atLeftTo(agregar, 10);
		
		Combo ordenarListaCombo = new Combo(agregar, SWT.NORMAL);
		...
		FormDatas.attach(ordenarListaCombo).atTopTo(ordenarLista, 10).atLeftTo(agregar, 10).withHeight(23).withWidth(170);
		...
		FormDatas.attach(TipoLista).atTopTo(agregar,15).atLeftTo(nombreListaText, 50);
		
		Combo TipoListaCombo = new Combo(agregar, SWT.NORMAL);
		...
		FormDatas.attach(TipoListaCombo).atTopTo(TipoLista, 10).atRight(0).atLeftTo(nombreListaText, 50).withHeight(23).withWidth(150);
		this.controls.put("tipoListaCombo", TipoListaCombo);
			
		
		Button guardar = new Button(agregar, SWT.PUSH);
		guardar.setText("Guardar");	 
		FormDatas.attach(guardar).atBottom(0).atRight(180);
		...
		FormDatas.attach(myLists).atTopTo(result, 10).atLeftTo(result, 20);
			
		final Button publicLists = new Button(result, SWT.ARROW_DOWN);
		publicLists.setText("Listas Publicas");
	    FormDatas.attach(publicLists).atTopTo(result, 10).atLeftTo(myLists, 0);
	    
	    
	     
	    final Button addLists = new Button(result, SWT.CONTROL);
	    addLists.setText("Nueva Lista");
	    FormDatas.attach(addLists).atTopTo(myLists, 5).atLeftTo(result, 20);
	    
		
		final CRUDTreeTable myListsTable = new CRUDTreeTable(result, SWT.NONE);

		FormDatas.attach(myListsTable).atTopTo(addLists, 5).atRight(0).atLeft(0).withHeight(200);
		...
		FormDatas.attach(publicListsTable).atTopTo(myLists,0).atRight(0).atLeft(0).withHeight(200);
