...
public class _CirculationRuleEditionArea implements IEditableArea {

	private Map<String, Control> controls;
	
	public _CirculationRuleEditionArea() {
	...
	public Composite createUI(final Composite parent, IGridViewEntity entity, IVisualEntityManager manager) {
		...
		String[] items = providerTypeCombo.getItems();
		int pos = -1;
		for (int i = 0; i < items.length; i++) {
			if( items[i] == fdtEntity.getProviderType()){
				pos = i;
			}
		}
		providerTypeCombo.select(pos);
