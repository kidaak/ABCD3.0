...
public class CustomControlDecoration {
	...
	private void setListMapControlDecorations(
			Map<String, ControlDecoration> listMapControlDecorations) {
		this.listMapControlDecorations = listMapControlDecorations;
	}

	public void createDecorator(DecoratedControl[] decoratedControls,
			DecoratorType decoratorType, int option) {
		switch (decoratorType) {
		case REQUIRED_FIELD: {
			CreateDecorateToRequeredField(decoratedControls[0], true);
			break;
		}
		case ERROR: {
			CreateDefaultErrorDecorateToField(decoratedControls[0]);
			break;
		}
		case WARNING: {
			CreateDefaultWarningDecorateToField(decoratedControls[0]);
			break;
		}
		case INFORMATION: {
			CreateDefaultInformationDecorateToField(decoratedControls[0]);
			break;
		}
		case CONTENT_PROPOSAL: {
			CreateDefaultContentProposalDecorateToField(decoratedControls[0]);
			break;
		}
		case LETTERS_ONLY: {
			CreateDecorateToOnlyLettersField(decoratedControls[0]);
			break;
		}
		case NUMBER_ONLY: {
			CreateDecorateToOnlyNumberField(decoratedControls[0]);
			break;
		}
		case ALPHA_NUMERIC: {
			CreateDecorateToAlphaNumericField(decoratedControls[0]);
			break;
		}
		case EMAIL: {
			CreateDecorateToEMailField(decoratedControls[0]);
			break;
		}
		case DATE_RANGE: {
			DecoratedControl auxDecoratedControl = decoratedControls[0];
			DecoratedControl auxDecoratedControl2 = decoratedControls[1];
			CreateDecorateToRangeDate(
					(DateTime) auxDecoratedControl.getDecorateControl(),
					(DateTime) auxDecoratedControl2.getDecorateControl(),
					option, auxDecoratedControl.getDecorateControlKey(),
					auxDecoratedControl2.getDecorateControlKey());
			break;
		}
		default:
			break;
	 ...
	 */
	private void CreateDecorateToRequeredField(
			DecoratedControl decoratedControl, boolean visible) {
		String image = "DEC_REQUIRED";
		final ControlDecoration controlDecoration = CreateDecorateToFieldAs(
				decoratedControl.getDecorateControl(), image,
				decoratedControl.getDecorateControlKey(),
				decoratedControl.getText(), visible);
		decoratedControl.getDecorateControl().addListener(SWT.Modify,
				new Listener() {
					...
					public void handleEvent(Event arg0) {
						CustomValidation
								.ValidateFieldRequired(controlDecoration);
					}
				});
	}
	 ...
	 *            - visibilidad inicial del ControlDecoration.
	 */
	private ControlDecoration CreateDecorateToFieldAs(Control control,
			String image, String controlDecorationName, String description,
			boolean visible) {
		final ControlDecoration decoration = new ControlDecoration(control,
				SWT.LEFT);
	 ...
	 */
	private ControlDecoration CreateDefaultErrorDecorateToField(
			DecoratedControl decoratedControl) {
		String image = "DEC_ERROR";
		return CreateDecorateToFieldAs(decoratedControl.getDecorateControl(),
				image, decoratedControl.getDecorateControlKey(),
				decoratedControl.getText(), false);
	}
	 ...
	 *            - control a crearle el ControlDecoration.
	 */
	private ControlDecoration CreateDefaultWarningDecorateToField(
			DecoratedControl decoratedControl) {
	 ...
	 *            - control a crearle el ControlDecoration
	 */
	private ControlDecoration CreateDefaultInformationDecorateToField(
			DecoratedControl decoratedControl) {
	 ...
	 * 
	 */
	private ControlDecoration CreateDefaultContentProposalDecorateToField(
			DecoratedControl decoratedControl) {
	 ...
	 *            - control a crearle el Control Decoration.
	 */
	private void CreateDecorateToOnlyLettersField(
			DecoratedControl decoratedControl) {
		String image = "DEC_ERROR";
		final ControlDecoration controlDecoration = CreateDecorateToFieldAs(
				decoratedControl.getDecorateControl(), image,
		...
		decoratedControl.getDecorateControl().addListener(SWT.Modify,
				new Listener() {
					...
					public void handleEvent(Event arg0) {
						CustomValidation
								.ValidateFieldOnlyLetters(controlDecoration);
					}
				});
	}
	 ...
	 *            - control a crearle el Control Decoration.
	 */
	private void CreateDecorateToOnlyNumberField(
			DecoratedControl decoratedControl) {
		String image = "DEC_ERROR";
		final ControlDecoration controlDecoration = CreateDecorateToFieldAs(
				decoratedControl.getDecorateControl(), image,
		...
		decoratedControl.getDecorateControl().addListener(SWT.Modify,
				new Listener() {
					...
					public void handleEvent(Event arg0) {
						CustomValidation
								.ValidateFieldOnlyNumbers(controlDecoration);
					}
				});
	}
	 ...
	 *            - control a crearle el Control Decoration.
	 */
	private void CreateDecorateToAlphaNumericField(
			DecoratedControl decoratedControl) {
		String image = "DEC_ERROR";
		...
		decoratedControl.getDecorateControl().addListener(SWT.Modify,
				new Listener() {
					...
					public void handleEvent(Event arg0) {
						CustomValidation
								.ValidateFieldAlphaNumeric(controlDecoration);
					}
				});
	}
	 ...
	 *            - control a crearle el Control Decoration.
	 */
	private void CreateDecorateToEMailField(DecoratedControl decoratedControl) {
		String image = "DEC_WARNING";
		final ControlDecoration controlDecoration = CreateDecorateToFieldAs(
				decoratedControl.getDecorateControl(), image,
				decoratedControl.getDecorateControlKey(),
	...
	private void CreateDecorateToRangeDate(DateTime init, DateTime end,
			final int option, String controlDecorationName1,
			String controlDecorationName2) {
		String image = "DEC_ERROR";
		...
		init.addListener(SWT.Modify, new Listener() {
			...
			public void handleEvent(Event arg0) {
				...
				case 2:
					CustomValidation
							.ValidateRangeDateFromLessOrEqualThanTo(lista);
					break;
				}
			}
		});
		...
		getListMapControlDecorations().put(controlDecorationName1,
				decorationInit);
		getListMapControlDecorations().put(controlDecorationName2,
				decorationEnd);

	}

	public boolean AllControlDecorationsHide() {
		Collection<ControlDecoration> collection = listMapControlDecorations
				.values();
		for (Iterator<ControlDecoration> iterator = collection.iterator(); iterator
				.hasNext();) {
			ControlDecoration controlDecoration = (ControlDecoration) iterator
					.next();
