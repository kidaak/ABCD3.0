...
public class EditarDatosDeLaAdquisicionDelEjemplar implements IContributor {

	@Override
	public String contributorName() {
		// TODO Auto-generated method stub
		return "Editar Datos de la Adquisición del Ejemplar";
	}
	...
	public String getID() {
		// TODO Auto-generated method stub
		return EditarDatosDeLaAdquisicionDelEjemplar.class.getSimpleName()+"id13";
	}
	...
	public Control createUIControl(Composite parent) {
		...
		group.setText("EDITAR DATOS DE LA ADQUISICIOŃ DEL EJEMPLAR");
		...
		FormDatas.attach(datosEjemplar).atTopTo(group, 36).atLeft(46);
		...
		FormDatas.attach(ultimaAct).atTopTo(group,36).atLeftTo(tablaDatosEjemplar, 35);
		...
		FormDatas.attach(sugerido).atTopTo(tablaDatosDelEjemplar, 20).atLeft(33);
		...
		Button modificar = new Button(group, SWT.NONE);
		FormDatas.attach(modificar).atTopTo(tablaSugerido, 10).atLeft(390);
		modificar.setText(AbosMessages.get().BUTTON_MODIFY);

		Button eliminar = new Button(group, SWT.NONE);
		FormDatas.attach(eliminar).atTopTo(tablaSugerido, 10).atLeftTo(modificar,30);
		...
		FormDatas.attach(exportaHC).atTopTo(modificar, 10).atLeftTo(group,370);
		exportaHC.setText(AbosMessages.get().BUTTON_EXPORT_TO_EXCEL);

		Button exportaPDF = new Button(group, SWT.NONE);
		FormDatas.attach(exportaPDF).atTopTo(modificar, 10).atLeftTo(exportaHC,20);
