...
public class ContributorTrackerService {
	...
	public ContributorTrackerService(BundleContext context, IAbosLog log) {
		...
		this.serviceTracker = new ServiceTracker<IContributorFactory, IContributorFactory>(
				context, type, null) {

			@Override
			public IContributorFactory addingService(
					ServiceReference<IContributorFactory> reference) {
				...
				try {
					...
					for (int i = 0; i < trackerList.length; i++) {
						((IServiceTracker) trackerList[i]).addingService(
								reference, result.create());
					}
					return result;
				} catch (ClassNotFoundException e) {
					if (logger != null) {
						logger.log(AbosLogConstant.ERROR,
								"ERROR adding IContrbutorFactory service with class "
										+ reference.getProperty("class"), e);
					}
				} catch (AbosInstantiationException e) {
					if (logger != null) {
						logger.log(AbosLogConstant.ERROR,
								"ERROR traing to instantiate class with name"
										+ reference.getProperty("class"), e);
					}
				}
				return result;
			...
			public void removedService(
				...
				for (int i = 0; i < trackerList.length; i++) {
					...
					tracker.removedService(reference,
							tracker.getContributor(reference));
				}
	...
	public void addServiceTracker(IServiceTracker tracker) {
		...
		for (int i = 0; i < services.length; i++) {
			...
			try {
				trackerAdapter.addingService(reference,
						((IContributorFactory) services[i]).create());
			} catch (AbosInstantiationException e) {
				if (logger != null) {
					logger.log(AbosLogConstant.ERROR,
							"ERROR traing to instantiate class with name"
