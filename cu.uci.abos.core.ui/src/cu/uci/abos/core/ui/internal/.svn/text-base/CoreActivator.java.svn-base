...
public class CoreActivator implements BundleActivator, ILogListner {
	...
	public void start(BundleContext arg0) throws Exception {
		...
		logTracker.open();
		if (logService != null)
			logService.log(AbosLogConstant.INFO, "Starting bundle "
					+ bundleName + " version " + bundleVersion, null);
		registerContributorTrackerService(arg0, logService);
		if (logService != null)
			logService.log(AbosLogConstant.INFO,
					"Publishing service under class "
							+ ContributorTrackerService.class, null);
		registerServiceProvider(arg0);
		if (logService != null) {
			logService.log(AbosLogConstant.INFO,
					"Publishing service under class " + IServiceProvider.class,
					null);
			logService.log(AbosLogConstant.INFO, "Started bundle " + bundleName
	...
	public void stop(BundleContext arg0) throws Exception {
		logService.log(AbosLogConstant.INFO, "Stoped bundle " + bundleName
				+ " version " + bundleVersion, null);
	...
	private void registerServiceProvider(BundleContext context) {
		...
		serviceProviderRegistration = context.registerService(type,
				serviceProvider, null);
	}

	private void registerContributorTrackerService(BundleContext context,
			IAbosLog logService) {
		...
		trackerServiceRegistration = context.registerService(type, service,
				null);
	}
