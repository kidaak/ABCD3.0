...
public class ContributorFactoryImpl implements IContributorFactory {
	...
	public void injectServiceReference(
		...
		String className = (String) contributorServiceReference
				.getProperty(CLASS);
		moduleUIContributorClass = (Class<IContributor>) contributorServiceReference
				.getBundle().loadClass(className);
		setViewController((IViewController) contributorServiceReference
				.getProperty("viewController"));
	}
	...
	private Method getMethod(Class<? extends IContributor> clazz) {
		Method result = null;
		try {
			result = clazz.getDeclaredMethod("setViewController",
					IViewController.class);
