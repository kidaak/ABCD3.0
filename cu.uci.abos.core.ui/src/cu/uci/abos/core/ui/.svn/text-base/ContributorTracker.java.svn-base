...
public class ContributorTracker {
	...
	class ServiceTrackerImpl implements IServiceTracker {
		...
		public void removedService(
				final ServiceReference<IContributorFactory> reference,
				final IContributor service) {
			if (!display.isDisposed()) {
				display.asyncExec(new Runnable() {
					...
					public void run() {
						if (canRun(reference)) {
							ContributorTracker.this.removedService(reference,
									service);
						}
					}
				});
		...
		public void addingService(
			...
			if (!display.isDisposed()) {
				display.asyncExec(new Runnable() {
					...
					public void run() {
						if (canRun(reference)) {
							ContributorTracker.this.addingService(reference,
									service);
						}
					}
				});
	...
	private class ContributorTrackerServiceTracker
			extends
			ServiceTracker<ContributorTrackerService, ContributorTrackerService> {
		ContributorTrackerServiceTracker() {
			super(ServiceProviderUtil
					.getBundleContext(ContributorTracker.class),
	...
	private ContributorTrackerService getContributorTrackerService() {
		...
		ContributorTrackerService resulTrackerService = trackerServiceTracker
				.getService();
	...
	public void removedService(ServiceReference<IContributorFactory> reference,
			IContributor service) {
		// subclasses may override
	}

	public void addingService(ServiceReference<IContributorFactory> reference,
			IContributor service) {
		// subclasses may override
	}
}
